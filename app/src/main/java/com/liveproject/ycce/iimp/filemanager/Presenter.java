...
public class Presenter implements LoaderManager.LoaderCallbacks<List<File>> {
    private FileUI mView; //Our view.
    private FileHolder mModel; //Our model.
    ...
    public Presenter(FileUI mView) {
        this.mView = mView;
        mModel = new FileHolder();
        mData = new ArrayList<>();
        init();
    }
    ...
    public void listItemClicked(ListView l, View v, int position, long id) {
        ...
        if (fileClicked.isDirectory()) {
            ...
            openFile(Uri.fromFile(fileClicked),fileClicked.getAbsolutePath());
            v.getContext();
        }
    }
    ...
    private void openFile(Uri fileUri, String filepath) {
        ...
        if (mimeType != null) { //we have determined a mime type and can probably handle the file.
            try {
                ...
                Log.d("Presenter : ", "openFile : "+ filepath);
                mView.getActivity().setResult(1,intent);
